...
class TradeDecision(BaseModel):
    ...
    symbol: str = Field(..., description="The actual trading symbol (e.g. BTC, ETH, SOL). Must be a valid ticker symbol. DO NOT use generic terms like ANY, LOWEST_CONVICTION_POSITION, or ANY_EXISTING_POSITION.")
    action: str = Field(..., description="The **ONLY** allowed values are BUY, SELL, and HOLD. DO NOT use any other value.")
    # trade_type: str = Field(..., description="The **ONLY** allowed values are LONG and SHORT. DO NOT use any other value.")
    quantity: float = Field(..., description="The amount of the asset to trade. Must be a positive floating-point number.")
...
class PortfolioDecision:
    ...
    def __init__(self):
        self.trades_to_execute: List[Tuple[str, TradeAction, TradeType, float]] = []
        self.reasoning: str = ""
...
class PortfolioDecisionManager:
    ...
    async def make_portfolio_decision(
        ...
        if self.llm_client:
            try:
                ...
                logger.error(f"Failed to get AI portfolio decision: {e} - using rule-based decision")
            ...
            logger.warning(f"LLM client not configured - using rule-based decision")
            decision = self._make_rule_based_decision(
                current_positions,
                portfolio_metrics,
                available_cash,
    ...
    def _build_portfolio_analysis_prompt(
        ...
        for symbol, analysis in self.asset_analyses.items():
            ...
            if asset in current_positions:
                prompt_parts.append(
                    f"  ⚠️ CURRENTLY HOLDING: {current_positions[asset].trade_type.value.upper()} position"
                )
            else:
    ...
    def _convert_ai_decision(
        ...
        for trade in ai_decision.recommended_trades:
            try:
                # TODO: explore and clean up all possible actions and trade_type
                logger.info(f"AI trade decision: {trade}")
                action = TradeAction(trade.action.lower())
                # TODO: need to support sell long and buy short when starting to support options
                # trade_type = TradeType(trade.trade_type.lower())
                trade_type = TradeType.SHORT if action == TradeAction.SELL else TradeType.LONG
                trade.symbol = trade.symbol.upper().removesuffix('-USD') # TODO: hacky code

                if action is None or trade_type is None:
                    logger.warning(f"Skipping trade with empty action or trade type: {trade}")
                    continue

                # Validate trade
                asset = trade.symbol.replace("USDT", "").replace("-USD", "")
                if asset not in current_positions and len(current_positions) >= self.config.max_positions:
                    logger.warning(
                        f"Skipping trade for {trade.symbol} - position already reached the maximum limit"
                    )
                    continue

                if action != TradeAction.HOLD:
                    decision.trades_to_execute.append(
                        (trade.symbol, action, trade_type, trade.quantity)
                    )

            except Exception as e:
                logger.error(f"Failed to convert trade decision for {trade.symbol}: {e}", exc_info=True)
    ...
    def _make_rule_based_decision(
        ...
        for asset, position in current_positions.items():
            ...
            if symbol in self.asset_analyses:
                ...
                if pnl < 0 and analysis.recommended_action == TradeAction.SELL:
                    decision.trades_to_execute.append(
                        (symbol, TradeAction.SELL, position.trade_type, position.quantity)
                    )
                    trades_added += 1
        ...
        for symbol, analysis in self.asset_analyses.items():
            ...
            if (analysis.recommended_action == TradeAction.BUY and
                    portfolio_metrics["position_count"] < self.config.max_positions):
                value_per_trade = self.config.risk_per_trade * total_portfolio_value
                quantity = value_per_trade / analysis.current_price
                decision.trades_to_execute.append(
                    (symbol, TradeAction.BUY, analysis.recommended_trade_type, quantity)
                )
                trades_added += 1
